...
void showNeighborsList() {
	if (mydata->nbNeighbors > 0) {
		uint8_t i;
		if (mydata->flag_verbose) { printf("[Kilobot ID%d] : list_neighbors.     \t| POS\t| NEIGHBOR_ID\t| CAPTURE_TIME\t| DISTANCE\t| ISITANCHOR\t(Size = nbNeighbors = %d) \t(nbAnchors = %d)\n", kilo_uid, mydata->nbNeighbors, mydata->nbAnchors); }
		for (i = 0; i < mydata->nbNeighbors; i++) {
			if (mydata->flag_verbose) { printf("\t\t\t\t\t| %d\t| %d\t\t| %d\t\t| %d\t\t| %d\n", i, mydata->list_neighbors[i].id, mydata->list_neighbors[i].age, mydata->list_neighbors[i].distance, mydata->list_neighbors[i].flag_isItAnchor); }
		}	
	}
}
