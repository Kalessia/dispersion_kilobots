...
// d06_initAnchors_00.
// The objectif of this algorithm is to obtain the smallest number of anchors, by selecting the more 
// isolated or marginal kilobots of each swarm cluster.
// At the beginning, kilobots are still and collect their neighbors placed at any distance (from distance=0) in their list_neighbors.
// A kilobot becomes an anchor if it has 0 or only 1 neighbor at the end of the ticks_min_timeToCollectNeighbors period.
// A kilobot will not become an anchor if at least one in its list_neighbors is an anchor.
// The algorithm lasts for a kticks_max_electionTime period : before the last iteration, each kilobot's list_neighbors database
// is emptied, ready to start the dispersion algorithm.
...
//	- If the density of kilobots in the arena is too high, it is possible to not determine any anchor : algorithms
//	  d07_initAnchors_plusAlea and d08_initAnchors_plusCrowd try to fix this issue.
